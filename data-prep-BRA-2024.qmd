...
cols <- cols(
  ...
  `Taxi (min)` = col_double(),
  Desimp. = col_character(),
  Adicional = col_character()
)
taxi_ins <- this_fn |> 
  dir_ls(regexp = "Taxi_in_\\d{4}.csv$") |> 
  purrr::map(.f = function(.x) {
    readr::read_csv(.x, col_types = cols) |>
      mutate(Adicional = parse_double(Adicional, locale = locale("es", decimal_mark = ",")),
             "Taxi (min)" = parse_double("Taxi (min)", locale = locale("es", decimal_mark = ",")),
             "Desimp." = parse_double("Desimp.", locale = locale("es", decimal_mark = ",")))
  } ) |> 
  bind_rows()
...
taxi_outs <- this_fn |> 
  dir_ls(regexp = "Taxi_out_\\d{4}.csv$") |> 
  purrr::map(.f = function(.x) {
    readr::read_csv(.x, col_types = cols) |>
      mutate(Adicional = parse_double(Adicional, locale = locale("es", decimal_mark = ",")),
             "Taxi (min)" = parse_double("Taxi (min)", locale = locale("es", decimal_mark = ",")),
